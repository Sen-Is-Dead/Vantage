@echo off
REM Phase 4 local runner. Double-click in the Vantage folder. Logged to scripts\phase4_run.log.
REM Steps: tests, refresh the live season (quick), refit + predict next 5 GWs, then produce the
REM transfer / XI / captain recommendation for your actual squad. Nothing is submitted to FPL.
setlocal
set PYTHONUTF8=1
cd /d "%~dp0.."
set LOG=scripts\phase4_run.log
echo === Phase 4 run %DATE% %TIME% === > "%LOG%"
